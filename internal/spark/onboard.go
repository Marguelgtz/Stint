...
type Area struct { ID string; ExpectedEvidence []string }
...
type OnboardingPlan struct { ProfilePath string; DashboardURL string; ExpectedEvidence []string; Steps []string }

func CreateOnboardingPlan(dashboardURL string) OnboardingPlan {
	if dashboardURL == "" { dashboardURL = DefaultDashboardURL }
	unique := map[string]struct{}{}
	for _, area := range Areas { for _, evidence := range area.ExpectedEvidence { unique[evidence] = struct{}{} } }
	evidence := make([]string, 0, len(unique))
	for name := range unique { evidence = append(evidence, name) }
