...
func TestRenderHomeShowsResidentLaneContext(t *testing.T) {
	model := Model{
		...
		Inference: Inference{
			Refreshed: true, Available: true, ContextUsed: 155005,
			Clients: []ClientContext{
				{Key: "lane:0", Label: "lane 1 · retained", Tokens: 7005},
				{Key: "lane:1", Label: "lane 2 · processing", Tokens: 148000},
			},
		},
	}
	...
	for _, want := range []string{
		...
		"lane 1 · retained  7.0k · 3%",
		"lane 2 · processing  148.0k · 56%",
	} {
	...
	if strings.Contains(out, "client") {
		t.Fatalf("resident runtime context must not be labeled as clients:\n%s", out)
	}
}
...
func TestContextBarUsesStableDistinctLaneColors(t *testing.T) {
	...
	model := Model{
		...
		Inference: Inference{
			...
			Clients: []ClientContext{
				{Key: "lane:0", Label: "lane 1 · retained", Tokens: 50000},
				{Key: "lane:1", Label: "lane 2 · processing", Tokens: 50000},
			},
		},
	}
...
func TestContextBarShowsFreeCapacity(t *testing.T) {
	model := Model{
		...
		Inference: Inference{
			...
			Clients: []ClientContext{
				{Key: "lane:0", Label: "lane 1 · retained", Tokens: 70000},
				{Key: "lane:1", Label: "lane 2 · processing", Tokens: 20000},
			},
		},
	}
...
func TestContextBarClampsVisualWhenObservedResidencyExceedsCapacity(t *testing.T) {
	model := Model{
		...
		Inference: Inference{
			...
			Clients: []ClientContext{
				{Key: "lane:0", Label: "lane 1 · retained", Tokens: 60000},
				{Key: "lane:1", Label: "lane 2 · processing", Tokens: 50000},
			},
		},
	}
