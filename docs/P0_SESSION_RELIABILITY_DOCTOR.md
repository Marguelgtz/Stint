...
**Status:** Historical P0 design and implementation record; current-main port is
under review on `fix/grounding-session-safety-20260923`.
**Source implementation:** `fix/p0-ninfer-session-safety`, PR #76, based on
the NInfer artifact-fix line (#74).
**Current port base:** `9634bf762a2dc9021747eb786db7fd23ccab84e9`.
...
## What starting `main` already had at `9634bf7`

The baseline already included useful recovery and observability behavior:

- `stint status --refresh` has a shared read-only snapshot/telemetry layer for tunnel, watchdog, endpoint, SSH/runtime, GPU, inference, performance, and state freshness.
- The dynamic deadline watchdog reloads session state while waiting, rechecks under the lifecycle lock at deadline, and retries failed provider destruction.
- Startup persists recovery checkpoints and can resume an existing paid instance.

The baseline `stint down` still sent one destroy request and cleared local state
without confirming provider disappearance. It also lacked type-to-confirm,
durable session archives, exact instance-bound detached watchdog identity,
instance-scoped logs, lifecycle-owner metadata, bounded safe preemption, and an
active-session Doctor. The port adds these invariants around the existing
snapshot and recovery architecture. Historical #75 remains source/evidence only;
...
The historical run references below verify the source implementation on PR #76
at `792bb508dfcd7d64e293359dfbed7b497b10dafa`. They do not verify this clean
current-main port; this branch's local and exact-head GitHub checks are tracked
separately in the repository grounding plan and PR ledger.

### P0-A — Paid-resource control and teardown

- [x] Confirm instance disappearance before clearing state (current-main port; local focused tests passed).
- [x] Preserve session state when destruction is unconfirmed (current-main port; local focused tests passed).
- [x] Dynamic watchdog retries provider/API destruction failures (baseline behavior retained).
