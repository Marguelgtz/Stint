...
- with `--refresh`, an `INFERENCE LIVE` section: requests processing, queued requests, resident prompt depth, decode/prefill rates, cache reuse, speculative acceptance, and per-lane detail observed from the engine's read-only `/metrics` and `/slots` endpoints (never an inference request). Retained context is not active work, and lanes are not attributed to callers.
