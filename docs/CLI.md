...
| `stint status` | Setup | Shows local state and the active session |
...
Creates (or reuses) a dedicated ed25519 keypair under `~/.config/stint/ssh/`. `stint start` attaches the key to rented instances automatically; you add the public key to Vast once.
...
Verifies everything needed for live planning and paid start: Vast credentials, OpenSSH, the dedicated SSH key, and local port 8409.
...
Prints local Stint state (Vast provider, SSH key, Cline endpoint) and, when a session is recorded, the active instance: GPU, rate, checkpoint, last error, auto-destroy deadline, and the suggested next action.
...
3. **Compute is bounded.** The paid session deadline is enforced by a detached watchdog that survives the CLI process; `stint down` tears down early.
4. **Failures are resumable.** Anything that breaks after SSH is ready keeps the instance paid-and-reachable; `stint resume` reattaches instead of re-renting.
5. **Credentials stay local.** The Vast API key and SSH keypair live under `~/.config/stint/` with owner-only permissions.

## Session lifecycle

```text
no session ──start──▶ RUNNING ──crash / laptop close──▶ RESUMABLE
    ▲                    │                                  │
    │                    │ deadline / stint down             │ stint resume
    └────────────────────┴──────────────────────────────────┘
```

`stint status` always reports which state the local machine believes it is in and the single suggested next action.