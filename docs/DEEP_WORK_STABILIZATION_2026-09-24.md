...
The refreshed Codex reviews of PR #160 found additional gaps and added regressions before merge: the log-tail redactor now covers quoted JSON secret keys; every supported phase-setup path transfers the helper and phase setup fails before continuing if it is unavailable; and the NInfer observer waits for the first new session pointer (or pins an explicit resume) before writing to a session-specific evidence file. It no longer follows a mutable `latest` pointer after attaching. Sampling continues until supervisor teardown while retained rows obey both the row and byte caps. The R2 archive allow-list regression is invoked in both CI test paths, preserving R2's existing bounded role.
