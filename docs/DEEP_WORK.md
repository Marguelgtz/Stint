...
`deep.json` remains the readable current-state projection. Journal-aware runs also append versioned lifecycle facts to `run-events.jsonl` in the run's state directory. The session ID is the stable run ID; each new or resumed execution period gets a separate epoch ID, while event sequence numbers continue monotonically across the whole run. Version 1 records epoch start, landing start, and landed checkpoint transitions. It does not yet record task attempts, executor invocations, or verifier invocations.

Journal appends and projection writes share a per-run file lock. Stint appends and syncs the event before atomically writing `deep.json` with the event watermark. Loading state replays complete events after that watermark. A projection watermark ahead of the journal, a malformed complete event, or an invalid event order fails closed. Only a final unterminated append is quarantined as `run-events.incomplete-tail` and truncated; earlier complete events are left untouched. If a Git checkpoint exists but the landing event was not recorded, a resumed landing reuses the matching checkpoint rather than creating another commit.
