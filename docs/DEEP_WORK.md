...
`deep.json` remains the readable current-state projection. Journal-aware runs also append versioned lifecycle facts to `run-events.jsonl` in the run's state directory. The session ID is the stable run ID; each new or resumed execution period gets a separate epoch ID, while event sequence numbers continue monotonically across the whole run. Version 1 records epoch start, landing start, and landed checkpoint transitions, with bounded task-state counts on each event and compute-instance identity on each epoch event. It does not yet record task attempts, executor invocations, or verifier invocations.

Journal appends and projection writes share a per-run file lock. For a resume, bounded context changes such as executor settings and compute binding are first persisted at the previous event watermark; the old deadline remains in place until the epoch event records its replacement. Stint then appends and syncs the event before atomically projecting the lifecycle transition and advancing the watermark. Loading state replays complete events after that watermark. A projection watermark ahead of the journal, a malformed complete event, or an invalid event order fails closed. Only a final unterminated append is quarantined as `run-events.incomplete-tail` and truncated; earlier complete events are left untouched. If a Git checkpoint exists but the landing event was not recorded, a resumed landing reuses the matching checkpoint rather than creating another commit.

Legacy `deep.json` remains readable without fabricated history. Its first journal-aware resume begins at sequence 1 with an explicit `legacy_resume` epoch boundary. The current A3 `missionOutcome` remains a derived projection and is deliberately not stored as a RunEvent conclusion; the journal records the observed landing and checkpoint facts. This keeps the existing `verified` task state from being promoted into a new durable `accepted` claim before Objective C defines that contract.
