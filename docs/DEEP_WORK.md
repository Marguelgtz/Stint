...
Legacy `deep.json` remains readable without fabricated history. Its first journal-aware resume begins at sequence 1 with an explicit `legacy_resume` epoch boundary. In B1, A3 `missionOutcome` remains a derived `deep.json` projection: `run.landed` records the operational landing and checkpoint, not mission success. B1 also does not journal final-verifier results or their provenance; the B2 verification-record layer must establish how those facts relate durably to mission outcome. This is separate from Objective C task acceptance, and no legacy `verified` task is promoted into a new `accepted` claim.
