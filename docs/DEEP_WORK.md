...
`--hours` is the explicit paid-compute opt-in. Without an active session and without `--hours`, `stint deep start` fails rather than renting unexpectedly. If a READY Stint session already exists, omit compute-provisioning flags and the command reuses that session. Supplying new-session cost/runtime flags while a READY session exists fails instead of silently ignoring them.
