...
func interruptRecordedWatchdog(paths config.Paths, lockPath string) (bool, error) {
	...
	if !lifecycleOwnerStillHoldsLock(owner, lockPath) || !watchdogProcessMatches(owner.PID, state.InstanceID) {
		return false, nil
	}
...
var watchdogProcessMatches = func(pid int, instanceID int64) bool {
	return processCommandMatches(pid, currentExecutablePath(), []string{"_watchdog", strconv.FormatInt(instanceID, 10)})
}
...
func lifecycleOwnerStillHoldsLock(owner lifecycleLockOwner, lockPath string) bool {
	...
	if owner.Executable != "" {
		if !processExecutableMatches(owner.PID, owner.Executable) {
			return false
		}
	} else {
		return false
