...
func bootstrapNInfer(ctx context.Context, paths config.Paths, state sessionstate.State) error {
	...
	fmt.Println("NInfer has no prebuilt Linux archive; Stint builds the pinned native server and falls back automatically when the host toolchain is incompatible.")
...
"$bin" --help >/dev/null
`, ninferSourceCommit, ninferSourceCommit, ninferSourceRepository, ninferSourceCommit, ninferSourceCommit)
}
