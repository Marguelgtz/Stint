...
func diagnoseActiveSession(paths config.Paths, state sessionstate.State) activeDoctorReport {
	...
	inputs.TunnelRunning = sessionTunnelRunning(paths, state)
	inputs.EndpointHealthy = snapshot.Health.Endpoint.Healthy
	inputs.WatchdogRunning = sessionWatchdogRunning(state)
	// PID existence alone can mistake an unrelated process that reused a stale
	// PID for Stint's tunnel or deadline watchdog. Doctor reports only verified
	// executable and argument matches as running.
	snapshot.Health.Tunnel.Running = inputs.TunnelRunning
	snapshot.Health.Watchdog.Running = inputs.WatchdogRunning
