...
// inferFromEpoch fills concurrency, per-lane depth, and ratio fields from a
...
func inferFromEpoch(result *inferenceTelemetry, epoch inferenceEpoch) {
	...
	// Agents means requests actively processing. A retained or resident prompt
	// is runtime context, not evidence of an active caller.
	result.Agents = result.Processing
	result.CacheReuseRatio = inferenceCacheReuseRatio(epoch.Counters)
	if _, hasPrefillCounter := epoch.Counters[metricPromptTokensTotal]; hasPrefillCounter {
		if _, isNInfer := epoch.Counters[metricNInferPrefixCacheHit]; isNInfer {
			// NInfer's shared prompt counter is uncached prompt work. Its cache-hit
			// counter must be included in the denominator to report a true reuse share.
			result.PrefillTokensKind = "uncached"
		} else {
			result.PrefillTokensKind = "total"
		}
	}
...
func inferenceCacheReuseRatio(counters map[string]float64) *float64 {
	if cached, isNInfer := counters[metricNInferPrefixCacheHit]; isNInfer {
		uncached, ok := counters[metricPromptTokensTotal]
		if !ok || cached < 0 || uncached < 0 {
			return nil
		}
		...
		ratio := math.Min(1, cached/total)
		return &ratio
	}
	cached, cacheCounterPresent := counters[metricPromptCachedTotal]
	total, totalCounterPresent := counters[metricPromptTokensTotal]
	if !cacheCounterPresent || !totalCounterPresent || cached < 0 || total <= 0 {
		return nil
	}
	ratio := math.Min(1, cached/total)
	return &ratio
