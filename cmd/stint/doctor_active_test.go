...
func TestDoctorClassifiesAnonymousLifecycleOwnerForReadySession(t *testing.T) {
	got := classifyDoctorInputs(doctorInputs{
		ProviderReachable: true,
		InstancePresent:   true,
		ProviderStatus:    "running",
...
func TestDoctorWatchdogFailurePrecedesProviderBootProgress(t *testing.T) {
	got := classifyDoctorInputs(doctorInputs{
		ProviderReachable: true,
		InstancePresent:   true,
	...
	if got.Diagnosis != doctorWatchdogMissing || got.Severity != "SAFETY" {
		t.Fatalf("classification = %+v, want WATCHDOG_MISSING/SAFETY while provider is booting", got)
	}
}
