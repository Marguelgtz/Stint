...
func TestDownYesPreemptsRecordedWatchdogHoldingLifecycleLock(t *testing.T) {
	...
	oldWatchdogMatches := watchdogProcessMatches
	watchdogProcessMatches = func(pid int, instanceID int64) bool {
		return pid == cmd.Process.Pid && instanceID == state.InstanceID
	}
	t.Cleanup(func() { watchdogProcessMatches = oldWatchdogMatches })
