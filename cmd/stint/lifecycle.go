...
func runStart(args []string) error {
	...
	offers, err := client.SearchOffers(searchCtx, profile, vast.SearchOptions{Hours: hours, Limit: 250, StorageGB: profile.Session.StorageGB})
	searchCancel()
	if err != nil {
		return err
	}
	...
	if err := client.AttachSSHKey(rootCtx, instanceID, publicKey); err != nil {
		return err
	}
	...
	remoteCommand := fmt.Sprintf(
		"mkdir -p /workspace/stint; nohup /app/llama-server -hf %s --alias %s --host 127.0.0.1 --port %d -ngl 99 -c 32768 --flash-attn on > /workspace/stint/llama.log 2>&1 < /dev/null &",
		interactiveModelRef, interactiveModelAlias, clineRemotePort,
	)
	...
	fmt.Println("Downloading/loading model; waiting for the local OpenAI endpoint...")
	if err := waitForModel(rootCtx, 18*time.Minute); err != nil {
		return err
	}
	state.Status = "READY"
	if err := sessionstate.Save(paths, state); err != nil {
...
func signalContext() (context.Context, context.CancelFunc) {
	...
	// os.Interrupt is sufficient for Ctrl-C. SIGTERM is handled by the process default
	// if the program is externally terminated before a session is created.
	_ = ch
	return ctx, cancel
}
...
func waitForSSHMetadata(ctx context.Context, client *vast.Client, id int64, timeout time.Duration) (vast.Instance, error) {
	...
	for {
		...
		if time.Now().After(deadline) {
			if err != nil {
				return vast.Instance{}, fmt.Errorf("timed out waiting for SSH metadata: %w", err)
			}
...
func waitForModel(ctx context.Context, timeout time.Duration) error {
	...
	for {
		...
		if time.Now().After(deadline) {
			return errors.New("timed out waiting for Qwen endpoint; run stint down to ensure compute is destroyed")
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
